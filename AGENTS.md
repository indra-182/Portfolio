...
- Blog Section: ISR fetch from `https://blog-seven-sandy-41.vercel.app/api/posts?limit=6`
  - Server Component, timeout 3s, fallback empty/cache
  - Card click → redirect to `https://blog-seven-sandy-41.vercel.app/posts/[slug]`
...
interface BlogPost {
  ...
  coverImage?: string; date: string;
  category: string; tags: string[];
  readingTimeMinutes: number; readingTime: string;
  url: string;
}
```
